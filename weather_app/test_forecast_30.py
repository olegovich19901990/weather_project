...
from weather_app.weather import get_forecast_30_days_units
...
def test_forecast_30_days_units(coordinates):
    ti = time.perf_counter()
    lat, lon = coordinates
    cnt = get_random_cnt()
    units = "metric"
    logger.info(f"Testing 30-day forecast for ({lat}, {lon}) - {cnt} days with units={units}")
    response = get_forecast_30_days_units(lat, lon, cnt, units)
    duration = (time.perf_counter() - ti) * 1000
    logger.info(f"Response status: {response.status_code}, time: {duration:.2f}")
    assert response.status_code == 200
    data = response.json()
